...
mod node_config_plumbing_tests {
    ...
    use crate::config::{ConfigArgs, ConfigPathsArgs};
    use crate::local_node::OperationMode;
    #[cfg(feature = "pulley")]
    use crate::wasm_runtime::engine::Engine;

    fn local_test_config_args(dir: &std::path::Path) -> ConfigArgs {
        ConfigArgs {
    ...
    async fn from_node_config_copies_use_pulley() {
        ...
        {
            let engine =
                Engine::create_backend_engine(&RuntimeConfig::from_node_config(&cfg)).unwrap();
